...
KeyboardPage::KeyboardPage(AppContext &ctx) : _ctx(ctx), _slotActions(ctx) {
  _list.setSource([&]() { return (int)SLOT_COUNT + 2; },
                  [&](int i, char *buf, int n) {
                    if (i == (int)SLOT_COUNT) {
                      snprintf(buf, n, "BLE: %s",
                               _ctx.fsm.inKeyboardMode() ? "ON " : "OFF");
                      return;
                    }
                    ...
                    const char *tag =
                        sl.occupied ? (_ctx.slots.active() == (uint8_t)i &&
                                               _ctx.ble.status() ==
                                                   BleHidStatus::CONNECTED
                                           ? "*"  // active + connected
                                           : "+") // bonded
                                    : "-";        // empty
...
void KeyboardPage::render(Canvas &c, int yTop) {
  ...
  // Footer: live BLE status at scale 1 so it fits the 122-px-wide portrait
  // panel.
