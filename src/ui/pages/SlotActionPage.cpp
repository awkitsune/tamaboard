...
struct ActionSet { const char* const* labels; int count; };

static ActionSet pick(const AppContext& ctx, uint8_t idx) {
    const auto& sl = ctx.slots[idx];
    if (!sl.occupied) return { LABELS_EMPTY, 2 };
    bool connected = ctx.slots.active() == idx
                  && ctx.ble.status() == BleHidStatus::CONNECTED;
    if (connected) return { LABELS_ACTIVE, 2 };
    return { LABELS_INACTIVE, 3 };
}

SlotActionPage::SlotActionPage(AppContext& ctx) : _ctx(ctx) {
    _list.setSource(
        [&]() { return pick(_ctx, _slotIdx).count; },
...
void SlotActionPage::render(Canvas& c, int yTop) {
...
void SlotActionPage::onLongPress(PageStack& nav) {
    ...
    if (sel >= as.count) { nav.popModal(); return; }
