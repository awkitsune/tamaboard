...
static State parseStateName(const String& s) {
    if (s == "EAT")      return State::EAT;
    if (s == "PLAY")     return State::PLAY;
    if (s == "SLEEP")    return State::SLEEP;
    if (s == "KEYBOARD") return State::KEYBOARD;
    return State::IDLE;
}

static Controller::SlotAction parseSlotAction(const String& s) {
    if (s == "pair")   return Controller::SlotAction::PAIR;
    if (s == "forget") return Controller::SlotAction::FORGET;
...
static void onMessage(const char* data, size_t len) {
    ...
    if (t == "action") {
        ...
        _ctrl->slotAction((uint8_t)fieldInt(s, "i"), parseSlotAction(fieldString(s, "action")));
    } else if (t == "clock") {
