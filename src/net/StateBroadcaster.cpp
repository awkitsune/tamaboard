...
static String buildState(PetFSM &fsm, BleHid &ble, DeviceSlots &slots) {
  ...
  j += ",";
  j += "\"ble\":\"";
  j += bleName(ble.status());
  j += "\",";
  j += "\"ble_slot\":";
  j += a;
  j += ",";
  j += "\"ble_slot_name\":\"";
  j += (sl.occupied ? sl.name : "");
  j += "\",";
  j += "\"clock_synced\":";
  j += (Clock::synced() ? "true" : "false");
  j += "}";
  return j;
}
...
static String buildSlots(BleHid &ble, DeviceSlots &slots) {
  ...
  for (uint8_t i = 0; i < SLOT_COUNT; i++) {
    ...
    bool connected =
        (slots.active() == i && ble.status() == BleHidStatus::CONNECTED);
    if (i)
      j += ",";
    j += "{\"name\":\"";
    j += sl.name;
...
void StateBroadcaster::pushState() {
  _ws.textAll(buildState(_fsm, _ble, _slots));
}
void StateBroadcaster::pushSlots() { _ws.textAll(buildSlots(_ble, _slots)); }
void StateBroadcaster::pushPairing(uint32_t pk) {
  _ws.textAll(buildPairing(pk));
}
void StateBroadcaster::pushAll() {
  pushState();
  pushSlots();
}
