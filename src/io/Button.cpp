...
void Button::_taskFn(void *arg) {
  ...
  while (true) {
    ...
    // Drain rapid bounces: consume all further edges within the debounce
    // window.
...
void Button::begin() {
  ...
  // 8 KB: callback chain runs onFsmStateChange → WiFi.mode() +
  // esp_light_sleep_start(), both of which are stack-heavy (WiFi driver uses
...
void Button::suppressNextEvent() { s_suppress = true; }
