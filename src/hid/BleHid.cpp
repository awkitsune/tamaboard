...
static const uint8_t HID_REPORT_MAP[] = {
    ...
    0x81,
    0x00, //   Input (Data, Array, Absolute) — keycodes[6]
    0xC0, // End Collection
    // Mouse — report ID 2
    0x05,
    0x01, // Usage Page (Generic Desktop)
    ...
    0x02, //     Report Count (2)
    0x81,
    0x06, //     Input (Data, Variable, Relative) — X, Y
    0xC0, //   End Collection
    0xC0, // End Collection
};
...
void BleHid::end() {
  ...
  // PC=0x00000000). The stack stays live; begin() skips re-init on _server !=
  // nullptr.
  _setStatus(BleHidStatus::IDLE);
}

void BleHid::key(uint8_t keycode, bool down) {
  if (!_keyReport || _status != BleHidStatus::CONNECTED)
    return;
  ...
  _keyReport->setValue(report, sizeof(report));
  _keyReport->notify();
  // One notification per call. The web UI controls timing via
  // pointerdown/pointerup.
}

void BleHid::mouseMove(int8_t dx, int8_t dy) {
  if (!_mouseReport || _status != BleHidStatus::CONNECTED)
    return;
...
void BleHid::mouseClick(uint8_t button) {
  if (!_mouseReport || _status != BleHidStatus::CONNECTED)
    return;
  // button=0 means button-up (all released); any other value sets those button
  // bits. The web UI sends down on pointerdown and up (btn=0) on pointerup —
  // one notify each.
...
void BleHid::selectSlot(uint8_t i) {
  if (i >= SLOT_COUNT || !_slots[i].occupied)
    return;
  _slots.setActive(i);
  _slots.save();
  if (_server && _server->getConnectedCount())
...
void BleHid::forgetSlot(uint8_t i) {
  ...
  if (_slots[i].occupied) {
    NimBLEDevice::deleteBond(_slots[i].addr);
    _slots[i].occupied = false;
    strlcpy(_slots[i].name, (String("Slot ") + (i + 1)).c_str(),
...
void BleHid::_startAdvertising() {
  if (!_hid) {
    Serial.println("[BleHid] _startAdvertising called before begin()");
    return;
  }
  NimBLEAdvertising *adv = NimBLEDevice::getAdvertising();
  adv->stop();
  ...
  adv->addServiceUUID(_hid->getHidService()->getUUID());
  // Always undirected. Directed advertising (with peer addr) times out in 1.28
  // s and silently stops; macOS also uses Resolvable Private Addresses that
  // change over time, so the stored peer addr quickly becomes stale.
  adv->start(0);
...
void BleHid::onDisconnect(NimBLEServer *, NimBLEConnInfo &, int reason) {
  ...
  if (!_active)
    return; // end() was called — don't re-advertise
  _startAdvertising();
}
...
void BleHid::onAuthenticationComplete(NimBLEConnInfo &info) {
  ...
  if (_passkeyCallback)
    _passkeyCallback(0); // 0 = paired, clear PIN screen
}
